...
Node *delete(Node *r, char *bsx)
{
    ...
    if (cmp > 0)
        r->rightChild = delete (r->rightChild, bsx);
    else if (cmp < 0)
        r->leftChild = delete (r->leftChild, bsx);
    else
    {
...
void inOrder(Node *r)
{
    ...
    inOrder(r->leftChild);
    // if (r->inout == 1 ){
    printf("%-15s %d-%02d-%02d %02d:%02d:%02d\n", r->bsx, r->dt.year, r->dt.month, r->dt.day, r->dt.hour, r->dt.minute, r->dt.second);
    //}
...
//...............Pre-chuc nang 6.............
Cars6_t *makeProfile6(DateTime dt, char *bsx, int inout)
{
    Cars6_t *node = (Cars6_t *)malloc(sizeof(Cars6_t));
    node->dt = dt;
    strcpy(node->bsx, bsx);
    node->inout = inout;
    node->next = NULL;
...
{
    Cars6_t *profile = makeProfile6(dt, bsx, inout);
    if (listEmpty6())
    {
        first6 = profile;
        last6 = profile;
...
void saveFile(char *filename)
{
    ...
    for (Cars6_t *p = first6; p != NULL; p = p->next)
    {
...
void trace()
{
    ...
    if (cnt == 0)
        printf("Xe khong gui tai bai xe\n");
}
...
int main()
{
    ...
    do
    {
        ...
        case 5:
            ...
            break;
        case 6:
            saveFile("change_log.txt");
            break;
        case 7:
            savePresentLog("log_present.txt");
            break;
        case 8:
            trace();
            break;
